...
# string = "....8....27.....54.95...81...98.64...2.4.3.6...69.51...17...62.46.....38....9...."
        ...
        if(pzl[index] == "."):
            ...
            for nbr in dictionary3[index]:
                if pzl[nbr] in pos:
                    pos.remove(pzl[nbr])
            for each in pos:
                dictSpots[each].append(index)
            if len(pos) < least:
                least = len(pos)
                smallInd = index
                final = set(pos)
    return smallInd, final, dictSpots

def getLeastSpots(dictSpots):
    least = 1000
    final = []
    poss = ""
    for each in dictSpots:
        if len(dictSpots[each]) != 0 and len(dictSpots[each]) < least:
            least = len(dictSpots[each])
            final = dictSpots[each][:]
            poss = each
    return final, poss
...
def bruteForce(pzl,index,pdList,count):
    if "." not in pzl:
        return pzl
    position,poss,dictSpots = getLeastPos(pzl,pdList)
    # print(position)
    # print(poss)
    posSpots,thePoss = getLeastSpots(dictSpots)
    # print(posSpots,thePoss)
    #tmp = pdList[:]
    #tmp.remove(position)
    index = position
